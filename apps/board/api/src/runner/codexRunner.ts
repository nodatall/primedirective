...
export class CodexRunner {
  run(prompt: string, worktreePath: string, sink: RunnerSink): ChildProcessWithoutNullStreams {
    assertCodexCwd(worktreePath, worktreePath);
    const child = spawn('codex', ['exec', '--json', '--full-auto', '--sandbox', 'workspace-write', prompt], { cwd: worktreePath, shell: false });

...
- Accepted one-shot trigger phrase: `begin task <task-id> in <prd-key> --one-shot`.
- Resolve files from `<prd-key>`:
  - Task list: `tasks/tasks-prd-<prd-key>.md`
  - PRD reference: `tasks/prd-<prd-key>.md`
- If `<task-id>` or `<prd-key>` is missing, or if either file does not exist, stop and ask for clarification.
...
- One-shot mode (`--one-shot`) behavior:
  ...
  - Start from `<task-id>`, then continue through remaining not-yet-complete sub-tasks in `tasks/tasks-prd-<prd-key>.md` in file order until all are complete.

...
13. After the final implementation slice, a clean checklist, passing focused validation, runnable artifacts, generated files, or archived planning files are not terminal states by themselves. If the next actions are final full-branch review, review remediation, committing final changes, baseline comparison, PR creation, or the existing-branch no-PR final handoff, perform those actions instead of handing the list back to the user.
14. A user-facing "What Changed" plus "Validation" summary is allowed only after final review and finalization. Before that point, treat it as an attempted terminal handoff and replace it with the missing closeout action.

...
13. After the final implementation slice, a clean checklist, passing focused validation, runnable artifacts, or archived planning files are not terminal states by themselves. If the next actions are final full-branch review, review remediation, committing final changes, baseline comparison, PR creation, or the existing-branch no-PR final handoff, perform those actions instead of handing the list back to the user.
...
- Treat a handoff that says "implementation complete, tests passed, reports runnable, branch dirty/uncommitted, remaining work is review/commit/finalization" as a failed finalization attempt, not as an acceptable terminal outcome. Continue the closeout sequence unless the blocker is external and named.

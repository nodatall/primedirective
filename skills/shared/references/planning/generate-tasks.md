...
4. If `$plan-and-execute --pro-analysis` is active, verify `tasks/tmp/pro-analysis-<plan-key>.md` before generating tasks. If the memo is missing, lacks Oracle invocation evidence for a real run, lacks a Pro synthesis completion stamp, says anything other than `oracle_action_run: yes`, `oracle_extended_thinking: yes`, and `pro_synthesis_complete: yes`, or leaves adopted Pro findings unapplied/unmapped, hard-stop task generation and report the unmet Pro synthesis gate.

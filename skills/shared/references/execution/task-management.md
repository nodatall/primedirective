...
- After all task boxes are checked, keep the same no-handoff rule through closeout. Passing focused validation, successful report/artifact runs, archived planning files, or a clean implementation summary do not permit a terminal message while final full-branch review, review fixes, commits, final status/baseline checks, PR creation, or the documented no-PR existing-branch handoff remain undone.
...
8. Prefer existing tests. If no test covers the path, create a temporary probe under `/agent-scratch/` that imports or copies the relevant production code and exercises the changed behavior. Keep `/agent-scratch/` gitignored unless the probe is promoted into a real test.
...
21. After final validation, continue directly into the final full-branch review and remediation. A message that says the implementation is complete but review, commit, or finalization remains is an invalid one-shot handoff unless it names an external blocker.
22. Treat branch publication and PR creation as explicit finalization work, not implied behavior. If the environment supports native PR actions, use them; otherwise push with git and open the PR with a concrete provider CLI.
23. Run the hard finalization gate from `skills/shared/references/execution/finalization-gate.md` before terminal handoff. If it fails, complete the missing work or report the exact blocker instead of sending a completion message.

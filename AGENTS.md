...
- When a request uses `reference: <link or pasted text>`, treat it as "what can Prime Directive skills learn from this?" Inspect the reference, compare it to existing skill owners, and discuss only reusable lessons plus any proposed small owned-contract changes. Do not edit Prime Directive skills, shared contracts, validators, README text, or automation instructions during the initial reference pass unless the user explicitly asks to implement after that discussion. Do not give a broad summary or create a new skill unless asked.
